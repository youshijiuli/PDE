...
class TESET:
    def __init__(self, bounds, nx):
        ...
        self.nx = nx
        self.hx = [
            (self.bounds[0, 1] - self.bounds[0, 0]) / self.nx[0],
...
class FENET:
    ...
    def Unit(self):  # 生成所有单元，单元数目(M*N)
        ...
        self.Units_Nodes = np.zeros([self.Units_size, 4], np.int32)  # 每个单元有4个点，记录这4个点的编号
        self.Units_Int_Points = torch.zeros(
            self.Units_size,  # 划分成M*N个小区域，每个区域有4个积分点
            self.gp_num * self.gp_num,
            self.dim,
        )
        m = 0
        for i in range(self.nx[0]):
    ...
    def matrix(self):
        self.A = torch.zeros(
            self.Nodes_size, self.Nodes_size
        )  # self.Nodes_size = (M+ 1)*(N + 1)
        for m in range(self.Units_size):  # self.Units_size = M*N，第m个区域单元的4个积分点
            for k in range(4):
                ind0 = self.Units_Nodes[
                    m, k
                ]  # self.Units_Nodes = [M*N,4],第m个区域中第k个网格点，第k个基函数编号
                for l in range(4):
                    ind1 = self.Units_Nodes[
                        m, l
                    ]  # self.Units_Nodes = [M*N,4],第m个区域中第l网格点，第k个基函数编号
    ...
    def right(self, UU, FF):
        ...
        for j in range(self.nx[1] + 1):  # b = [(nx[0] + 1)*(nx[1] + 1),1]
            ...
            self.b[ind] = UU(
                self.Nodes[ind : ind + 1, :], [0, 0]
            )  # self.Nodes存储(M + 1)*(N + 1)个网格点
    ...
    def phi(self, X, order):  # [-1,1]*[-1,1]，在原点取值为1，其他网格点取值为0的基函数
        ...
        if order == [0, 0]:
            return (
                (ind00 * ~ind01 * ind10 * ~ind11).float()
                * (1 + X[:, 0])
                * (1 + X[:, 1])
                + (ind01 * ~ind02 * ind10 * ~ind11).float()
                * (1 - X[:, 0])
                * (1 + X[:, 1])
                + (ind00 * ~ind01 * ind11 * ~ind12).float()
                * (1 + X[:, 0])
                * (1 - X[:, 1])
                + (ind01 * ~ind02 * ind11 * ~ind12).float()
                * (1 - X[:, 0])
                * (1 - X[:, 1])
            )
        if order == [1, 0]:
            return (
                (ind00 * ~ind01 * ind10 * ~ind11).float() * (1 + X[:, 1])
        ...
        if order == [0, 1]:
            return (
                (ind00 * ~ind01 * ind10 * ~ind11).float() * (1 + X[:, 0])
    ...
    def solve(self, UU, FF):
        self.right(UU, FF)
        self.Nodes_V = torch.linalg.solve(self.A,self.b)  # 求解方程组得到所有网格点的值
...
# the error of FE is 5.003e-03,use time :4.71