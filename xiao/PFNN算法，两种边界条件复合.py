...
def U(X, order, prob):
    ...
    if prob == 2:
        ...
        if order[0] == 2 and order[1] == 0:
            return (
                (6 * X[:, 0]) * 0.5 * (torch.exp(2 * X[:, 1]) + torch.exp(-2 * X[:, 1]))
            )
        if order[0] == 1 and order[1] == 1:
            return (3 * X[:, 0] * X[:, 0] - 1) * (
                torch.exp(2 * X[:, 1]) - torch.exp(-2 * X[:, 1])
    ...
    if prob == 3:
        ...
        if order[0] == 1 and order[1] == 0:
            return (2 * X[:, 0]) * temp2 ** (-1) + temp1 * (-1) * temp2 ** (-2) * (
                2 * X[:, 0]
            )
        if order[0] == 0 and order[1] == 1:
            return (-2 * X[:, 1]) * temp2 ** (-1) + temp1 * (-1) * temp2 ** (-2) * (
                2 * X[:, 1]
            )
        if order[0] == 2 and order[1] == 0:
            return (
    ...
    if prob == 4:
        ...
        if order[0] == 1 and order[1] == 1:
            ...
            return ind * (4 * (X[:, 0] + 1) ** 3) * (temp * (-8 * X[:, 1])) + (
                1 - ind
            ) * (4 * (-X[:, 0] + 1) ** 3) * (temp * (-8 * X[:, 1]))
        if order[0] == 0 and order[1] == 2:
            ind = (X[:, 0] <= 0).float()
            return ind * ((X[:, 0] + 1) ** 4 - 1) * (
                temp * (64 * X[:, 1] * X[:, 1] - 8)
            ) + (1 - ind) * (-((-X[:, 0] + 1) ** 4) + 1) * (
                temp * (64 * X[:, 1] * X[:, 1] - 8)
            )
...
def A(X, order, ind):
    if order == 0:
        if ind == [1, 1]:
            return (X[:, 0] + X[:, 1]) * (X[:, 0] + X[:, 1]) + 1  # a11
        if ind == [1, 2]:
            return -(X[:, 0] + X[:, 1]) * (X[:, 0] - X[:, 1])  # a12
        if ind == [2, 1]:
            return -(X[:, 0] + X[:, 1]) * (X[:, 0] - X[:, 1])  # a21
        if ind == [2, 2]:
            return (X[:, 0] - X[:, 1]) * (X[:, 0] - X[:, 1]) + 1  # a22
    if order == 1:
        if ind == [1, 1]:
            return 2 * (X[:, 0] + X[:, 1])  # a11_x
        if ind == [1, 2]:
            return -2 * X[:, 0]  # a12_x
        if ind == [2, 1]:
            return 2 * X[:, 1]  # a21_y
        if ind == [2, 2]:
            return -2 * (X[:, 0] - X[:, 1])  # a22_y


def C(X, prob):
    return (
        A(X, 1, [1, 1]) * U(X, [1, 0], prob)
        + A(X, 0, [1, 1]) * U(X, [2, 0], prob)
        + A(X, 1, [1, 2]) * U(X, [0, 1], prob)
        + A(X, 0, [1, 2]) * U(X, [1, 1], prob)
        + A(X, 1, [2, 1]) * U(X, [1, 0], prob)
        + A(X, 0, [2, 1]) * U(X, [1, 1], prob)
        + A(X, 1, [2, 2]) * U(X, [0, 1], prob)
        + A(X, 0, [2, 2]) * U(X, [0, 2], prob)
    )


def NEU(X, n, prob):
    return (
        A(X, 0, [1, 1]) * U(X, [1, 0], prob) + A(X, 0, [1, 2]) * U(X, [0, 1], prob)
    ) * n[:, 0] + (
        A(X, 0, [2, 1]) * U(X, [1, 0], prob) + A(X, 0, [2, 2]) * U(X, [0, 1], prob)
    ) * n[
        :, 1
    ]
...
class INSET:
    def __init__(self, bound, nx, prob):
        ...
        self.hx = [
            (bound[0, 1] - bound[0, 0]) / nx[0],
            (bound[1, 1] - bound[1, 0]) / nx[1],
        ]
        self.size = nx[0] * nx[1]
        ...
        self.AM[:, 0, 0] = A(self.X, 0, [1, 1])
        self.AM[:, 0, 1] = A(self.X, 0, [1, 2])
        self.AM[:, 1, 0] = A(self.X, 0, [2, 1])
        self.AM[:, 1, 1] = A(self.X, 0, [2, 2])
...
class NETF(nn.Module):  # u = netg*lenthfactor + netf，此为netg，此netg逼近内部点取值
    def __init__(self):
        ...
        self.model = nn.Sequential(Res(2, 10), Res(10, 10), Res(10, 10))
...
def main():
    ...
    print(
        "testerror_mean = %.3e, testerror_std = %.3e"
