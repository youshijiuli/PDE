    ...
    def __init__(self, bound, nx, mode):
        ...
        if mode == "uniform":
            ...
            self.X[:, 0] = (
                bound[0, 0]
                + self.hx[0]
                + (bound[0, 1] - bound[0, 0] - 2 * self.hx[0]) * tmp[:, 0]
            )
            self.X[:, 1] = (
                bound[1, 0]
                + self.hx[1]
                + (bound[1, 1] - bound[1, 0] - 2 * self.hx[1]) * tmp[:, 1]
            )
        else:
...
class TESET:
    def __init__(self, bound, nx):
        ...
        self.hx = [
            (self.bound[0, 1] - self.bound[0, 0]) / self.nx[0],
...
class Net(torch.nn.Module):
    ...
    def forward(self, x):
        ...
        for i in range(self.layers_hid_num):
            ...
            temp = torch.eye(
                x.shape[-1], self.layers[i + 1], dtype=self.dtype, device=dev
            )
            x = h + x @ temp
        return self.fc[-1](x)
...
def loss_func_yp(nety, netu, lenth, inset):
    ...
    inset.J = (
        0.5 * ((inset.y - inset.yd) ** 2 + alpha * inset.u**2).mean() * inset.area
    )
    ...
    line = (
        inset.lam[:, 0:1] * inset.res_y
        + inset.lam[:, 1:2] * inset.ua
        + inset.lam[:, 2:3] * inset.ub
    ).mean() * inset.area
    nonline = (
        0.5
        * (
            inset.tau[0] * inset.F2
            + inset.tau[1] * inset.ua2
            + inset.tau[2] * inset.ub2
        )
        * inset.area
    )
...
def lam_para(inset, theta):
    ...
    temp = inset.lam[:, 0:3]
    inset.lam[:, 0:3] = (
        inset.lam[:, 0:3] + tmp * (inset.lam[:, 0:3] - inset.lam[:, 3:6])
    ).detach()
    inset.lam[:, 3:6] = temp
...
def train_yp(nety, netu, lenth, inset, optim, epoch):
    ...
    print(
        "epoch_yp: %d, loss_yp: %.3e,loss_obj:%.3e, time: %.2f"
    ...
    for it in range(epoch):
        ...
        ela = time.time() - st
        print(
            "epoch_yp: %d, loss_yp: %.3e, loss_obj:%.3e,loss_pde:%.3e,loss_iu:%.3e, time: %.2f"
            % (
                (it + 1),
                loss.item(),
                inset.J.item(),
...
def KKT_loss(nety, netu, lenth, inset, error_history, record=True):
    ...
    print(
        "pde_y_loss: %.3e,y_err: %.3e, u_err: %.3e, obj_value: %.5f"
...
for i in range(max_iters):
    ...
    if optimtype == "LBFGS":
        optim = torch.optim.LBFGS(
            itertools.chain(nety.parameters(), netu.parameters()),
            lr=lr,
            max_iter=100,
